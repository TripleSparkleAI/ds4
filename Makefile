...
clean:
	...
	rm -f tests/test_warmset

# The active tokenizer includes generated Unicode classes.
ds4.o ds4_cpu.o ds4_cpu_test_hooks.o: ds4_qwen4_unicode.inc

# THE HIPPOCAMPAL WARM SET.  Pure C, no CUDA, no model, no GPU: proves the
# session/day retention rules, the merge rule for two sessions that disagree
# about one expert, and the structural safety property that a warm-set entry
# carries identity and evidence and never a magnitude.  See ds4_warmset.h.
WARMSET_TEST := tests/test_warmset

tests/test_warmset: tests/test_warmset.c ds4_warmset.h
	$(CC) -O2 -g -Wall -Wextra -std=c11 -I. -o $@ tests/test_warmset.c

.PHONY: test-warmset
test-warmset: tests/test_warmset
	./tests/test_warmset
